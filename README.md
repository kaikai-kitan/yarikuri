...
- 🍳 **AIレシピ提案**: 冷蔵庫＋特売品の組合せから3件のレシピを提案、コスト・カロリー付き
- ⚙️ **提案の設定**: 何人分で作るか（1〜8人）と、優先する条件（安さ／カロリー控えめ／時短）を選べる。
  分量は人数分、`totalCost` と `calories` は1人前で返る。並び順はAI任せにせず、選んだ軸でサーバー側でも並べ直す
