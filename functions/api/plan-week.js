...
2. 次に、決めた料理に必要で、かつ冷蔵庫に無いものだけを shoppingList に挙げます。
   ...
   特売品がある場合は最安のスーパーを \`buyAt\` に指定してください。
3. 最後に、各日の材料を usedFromFridge と usedFromShopping に振り分けます。
   1つの食材（例：キャベツ1玉）を複数日に分けて使う場合は、\`usedFromFridge\` または \`usedFromShopping\` にその日の消費量（例：「キャベツ 1/2玉」）を含めて記載し、効率よく使い切るように計画してください。
...
{
  "shoppingList": [
    { "name": "1日目にまとめ買いする食材名", "amount": "買う量(例: 1個, 200g)", "buyAt": "購入先スーパー名(特売にあれば最安の店名を、なければ空文字)", "estimatedPrice": 概算円 }
  ],
  "days": [
    {
      "day": 1,
